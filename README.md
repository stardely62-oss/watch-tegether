...
- 🔍 **Быстрый поиск и импорт**: Интеграция с PoiskKino API ([poiskkino.dev](https://poiskkino.dev/)) — автозаполнение постеров, жанров, описаний и рейтингов. Бесплатный тариф предоставляет **200 запросов в сутки**, чего с большим запасом хватает для регулярного использования компанией друзей.
...
- 🔒 **Приватность и анонимность Telegram**: Защита личных данных участников. Настоящие @username, Telegram ID и аватарки Telegram **не раскрываются** другим участникам комнаты. При первом входе открывается полноэкранный экран создания профиля с указанием псевдонима (который можно сменить в любой момент в шапке), а аватары автоматически генерируются по инициалам с индивидуальной цветовой палитрой.
...
- 🔔 **Персональные уведомления в Telegram**: Мгновенная рассылка ботом всем участникам в ЛС при добавлении новых фильмов/сериалов, публикации оценок, комментариях и отметках «+ Я тоже посмотрел(а)» с кнопкой прямого перехода в приложение.

---

## 👑 Возможности администратора

Администраторы системы (указанные в `ADMIN_TELEGRAM_IDS` в `.env` или первый зарегистрированный пользователь) имеют расширенные права:

- 🚫 **Модерация и бан пользователей**: Блокировка (`Ban / Unban`) нежелательных участников. Заблокированный пользователь мгновенно теряет доступ к API и приложению.
- 🗑️ **Удаление пользователей**: Полное удаление профиля и связанных данных пользователя.
- 💬 **Модерация комментариев**: Права на удаление любых рецензий и комментариев (обычные пользователи могут удалять только свои).
- 🎬 **Управление медиатекой**: Редактирование и удаление карточек контента.
- ⚙️ **Администрирование Telegram-бота**: Настройка вещания анонсов в Telegram-чат (`TELEGRAM_CHAT_ID`), установка webhook и главного меню приложения.
...
> 💡 **Как получить API ключ PoiskKino**:
> Перейдите на сайт [poiskkino.dev](https://poiskkino.dev/), пройдите простую регистрацию и скопируйте созданный API ключ из личного кабинета. Бесплатный тариф предоставляет **200 запросов в сутки** — этого объёма более чем достаточно для поиска и добавления фильмов в кругу друзей.
...
| `POISKKINO_API_KEY` | API ключ Кинопоиска ([poiskkino.dev](https://poiskkino.dev/)) — Бесплатный тариф включает **200 запросов/сутки** | **Да** |
...
- Приватность профилей: Telegram ID и username хранятся только в защищенной базе на сервере (для работы сессий и рассылок бота) и не передаются в API клиентам.
